...
public partial class StatusLayerContext
{
    private int _countOfRunningBlockingTasks;
    private int _countOfRunningNonBlockingTasks;

    public StatusLayerContext(AppToastContext toastContext)
    {
    ...
    public static async Task<StatusLayerContext> CreateInstance()
    {
        await UiThreadSwitcher.ResumeBackgroundAsync();

        var factoryToast = await AppToastContext.CreateInstance();
        return new StatusLayerContext(factoryToast);
    }
